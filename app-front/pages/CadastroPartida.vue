        ...
        <b-card bg-variant="light" >
            <b-form @submit="NovaPartida">
                ...
                <b-form-group label="Imprensa:" label-for="imprensa-input" label-cols-sm="2" label-align-sm="left">
                    <b-form-select v-model="novaPartida.imprensa" :options="imprensaNome" size="sm" class="mt-3"></b-form-select>
                </b-form-group>
...
export default {
    async asyncData({ $axios }) {
        ...
        let imprensaNome = [
            { value: null, text: 'Por favor escolha um tipo de imprensa' },
        ];
        let evento = [
        ...
        const imprensaResponse = await $axios.$get('CadastroImprensa');
        imprensaResponse.forEach(element => {
            imprensaNome.push(element["nome"]);
        });
        ...
        return { time,tecnicoNome,imprensaNome,evento };
    },
